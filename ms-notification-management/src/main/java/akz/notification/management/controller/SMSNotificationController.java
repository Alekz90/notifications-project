...
import akz.notification.management.dto.SMSNotificationDto;
...
@Validated
@RestController
@RequestMapping("v1/sms-notifications")
@Tag(name = "SMS Notifications", description = "SMS notification endpoints management")
public class SMSNotificationController {

  private final INotificationService service;

  public SMSNotificationController(@Qualifier(SMS_SERVICE_NAME) INotificationService service) {
  ...
  public ResponseEntity<ResultDto<NotificationDto.Response>> create(
    @PathVariable long userId, @RequestBody @Valid SMSNotificationDto.Register notificationDto) {
    NotificationDto.Response response = service.create(userId, SMSNotificationDto.fromRecord(notificationDto));
    return ResponseEntity
      .created(CommonUtils.buildUriPost("/v1/sms-notifications/", response.id()))
  ...
  public ResponseEntity<ResultDto<NotificationDto.Response>> update(
    ...
    @RequestBody @Valid SMSNotificationDto.Register notificationDto) {
    NotificationDto.Response response = service.update(id, updatedAt, SMSNotificationDto.fromRecord(notificationDto));
